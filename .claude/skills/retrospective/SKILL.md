---
name: retrospective
description: "*· Periodic retrospective audit that breaks out of forward-only iteration to detect marginal-tuning drift, repeated failure patterns, skipped early signals, and abandoned routes worth reconsidering. Reads docs/03_benchmark_roadmap.md through docs/09_decisions_log.md, then produces an advisory review — it never overwrites the roadmap, running job, Track B promotion rule, or user-approved path. Trigger every 5 completed iterations, after 3 consecutive failures on the same path, after 2 failed Track B scale-ups, or whenever the user suspects the project is going in circles."
argument-hint: "<optional: focus area, e.g. 'check whether Path 2 is marginal' or 'audit last 10 iterations'>"
---
