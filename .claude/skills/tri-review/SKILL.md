---
name: tri-review
description: "B4· Run independent full-scope tri-review after result-log and before pivot. It directly invokes Claude, Codex, and Antigravity CLIs in parallel; every reviewer receives the same prompt and evaluates fairness, comparability, semantic success, leakage/reproducibility, architecture hypothesis, Track A/B decision, and next step toward SOTA. Failed reviewers are retried; 2 reviewers permit DEGRADED_REVIEW, 1 reviewer permits only SINGLE_REVIEW_CONTINUATION for ordinary non-claim iteration. Do not use subagents as reviewers."
argument-hint: "<experiment_id with result + SOTA target>"
---
...
CMD_C='bash .claude/skills/tri-review/scripts/reviewer_c_antigravity.sh '"$PROMPT"   # antigravity (agy)
