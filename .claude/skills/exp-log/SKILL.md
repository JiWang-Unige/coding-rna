---
name: exp-log
description: "*· After an experiment finishes (right after /result-log), write a dedicated structured per-experiment file docs/experiments/<exp_id>.md (hypothesis / architecture / data / config / result / findings / lineage), then regenerate docs/experiments/ATLAS.md via scripts/build_atlas.py — a categorized overview that groups every experiment by approach_family (e.g. pretrained-LM vs traditional-DL vs hybrid) into a progression tree, so you can see at a glance what was tried and how it advanced. Use after each /result-log; a hook nudges you."
argument-hint: "<exp_id just completed>"
---
