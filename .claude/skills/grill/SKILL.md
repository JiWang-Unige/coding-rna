---
name: grill
description: "A3.5· A deep two-phase exchange on the USER's research direction AFTER papers/code have been read (post /sota-inventory, pre /configure-project). Phase 1 co-develops the IDEA itself — steelman it, explore the hypothesis space, surface adjacent angles and the most interesting/surprising version (a genuine thinking partner, not just a critic). Phase 2 adversarially grills the technical specifics (grill-me Socratic pressure + devil's-advocate stress test) to turn it into concrete, defensible decisions before any GPU is spent — anti-sycophancy: no hand-wavy answers accepted. Re-invoke whenever the direction feels fuzzy or new papers shift it."
argument-hint: "<the direction/hypothesis to grill, e.g. 'replace HMM head with CRF'>"
---
