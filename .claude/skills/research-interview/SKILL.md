...
description: "A1· Deep interview to clarify a research direction, producing (a) a prompt to send to external deep research tools (ChatGPT / Perplexity / Perplexity / Claude deep research) and (b) an intent memo for the next-stage skill /research-synthesize. Auto-detects two modes — FRESH for new projects from a vague direction, and CONTINUATION for ongoing projects (reads CLAUDE.md / docs / findings.md / EXPERIMENT_TRACKER, summarizes project state inline, then interviews around current gaps instead of starting from zero). Use when starting a new research project from a vague direction, when re-engaging an existing project for a new attack direction, or when the user explicitly says \"let's plan the next deep research for this project\"."
argument-hint: "<vague research direction OR 'continue: <focus area>' for existing project>"
---
