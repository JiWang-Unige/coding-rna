---
name: pursue
description: "*· Supervised-autonomy driver toward a high-level goal (e.g. \"reach a SOTA model on metric X\"). Reads the machine-readable ACTIVE_GOAL.json contract and iterates experiments internally (plan → smart-sbatch → result-log → DETERMINISTIC validate gate → tri-review → pivot) WITHOUT requiring the user to hand-write each /goal. Crucially, continue/stop is decided by scripts/validate_goal.py (a deterministic script), NOT by agent self-judgment — so a failed or degenerate run can never silently pass; it halts and notifies the human. Use when the user wants goal-level autonomy (\"build the SOTA model, don't make me babysit each step\") but with a hard failure tripwire. Supports terminal goals and milestone goals via ACTIVE_GOAL.json `scope`."
argument-hint: "[optional: override goal text / point to a specific ACTIVE_GOAL.json]"
---
