...
description: "A4-pre· After Stage-A research has clarified the project (research-interview → synthesize → sota-inventory → grill), let the AI MATERIALIZE the persistent project contract instead of you hand-filling it at install time: fills CLAUDE.md §0-2 (scope/data/baseline) and §12-15 (compute/canonical commands) from the clarified docs + verified dossiers, fills cluster_config.yaml from LIVE cluster detection (sinfo/sacct), detects the conda env, and proposes ACTIVE_GOAL.json — all human-gated (propose diff → you confirm → write), then regenerates AGENTS.md. Re-invoke anytime to re-materialize (e.g. after /reframe or moving to a new cluster)."
argument-hint: "<optional: 'after grill' / 'refresh compute for baobab' / 'after reframe'>"
---
