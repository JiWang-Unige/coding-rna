...
description: "B1.5· Mandatory read-only pre-submit code review gate after /implement and before /smart-sbatch/full training. Use when training/eval/data/config/job scripts changed, when a run could invalidate results if code is wrong, or when Codex is the only available driver and must self-review with a separate checklist."
---
