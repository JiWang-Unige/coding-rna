...
description: "*· Upgrade an existing auto-research project/framework version in place, especially v3.x→v4.x or v4.x minor upgrades, while preserving research content. Use when scripts, skills, docs templates, hooks, driver shells, or compatibility contracts need migration without wiping docs/refs/wiki/runs or moving goalposts."
---
...
- `.claude/skills` 是 canonical skill layer；`.agents/skills` 与 `.codex/skills` 由 `scripts/build_codex_skills.py` 生成。
...
1. 新增/更新 canonical `.claude/skills/<skill>/SKILL.md`。
