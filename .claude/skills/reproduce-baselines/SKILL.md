...
description: "B0· Before writing ANY of our own model code, reproduce 1-2 verified SOTA baselines locally ONCE to nail the technical ground truth you cannot trust from the paper alone: exact metric computation, dataset rawness (pure raw vs FP-pre-filtered / homology-deduped), split scheme, preprocessing. Establishes the measurement ground truth + the basis for screen_anchor, and surfaces report-vs-reproduce gaps. Gate before /goal iteration. Use after /benchmark-roadmap picks paths, before /goal-prompt."
argument-hint: "<which SOTA(s) to reproduce, e.g. 'HyenaDNA on the chosen benchmark'>"
---
