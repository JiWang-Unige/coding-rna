---
name: pivot
description: "B5· Make a single final decision (continue / tune / scale / replace component / change backbone / change objective / abandon / return to literature / sanity check first) by CONSUMING the output of /tri-review. Pivot is the synthesizer, tri-review is the council — never run pivot before tri-review. It first records ALL tri-review conclusions and the directions each reviewer proposed, then converges to a single primary decision plus an optional parallel cohort of orthogonal directions. User strongly prefers architectural changes when gap to SOTA is large; do not default to parameter tuning. Use after /tri-review has been completed."
argument-hint: "<tri-review output or exp_id with result + SOTA gap>"
---
