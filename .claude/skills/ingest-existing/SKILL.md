...
description: "A0· Onboard a half-finished EXTERNAL research project into the framework. Fans out read-only subagents to systematically survey existing code / configs / results / notes / papers / data, reconstructs what was tried, what was concluded, and WHY, reports a structured digest for you to review, then co-decides the goal + workflow mode and populates docs/00-16 (human-gated) so the work continues inside the framework. Distinct from /research-interview CONTINUATION (which re-engages a project ALREADY in the framework) — this is a COLD import from raw prior material that has no framework docs yet. Use when you bring half-done research in to systematize / iterate / publish."
argument-hint: "<path(s) to the existing project/code/notes/results/manuscript to ingest>"
---
...
**按分区并行派 read-only subagent**（优先使用 `.claude/agents/project-cartographer`；每个只读一类材料，**只返回结构化摘要、不返回原文**；主对话只接收摘要表）：
...
- **Uses**: `.claude/agents/project-cartographer` read-only survey subagents（并行）、`scripts/artifact_registry.py`（建目录骨架 + 规整审计）、`refs/archive_source.sh`（归档已有论文）、`scripts/check_data.py`（数据泄漏初查）
