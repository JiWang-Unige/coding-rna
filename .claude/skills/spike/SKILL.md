---
name: spike
description: "*· Insert an isolated exploratory side-experiment at any time WITHOUT polluting the mainline. Uses a SPIKE-* exp_id namespace; the result is recorded but by default does NOT count toward mainline portfolio promotion / Track-B / the anti-tuning trajectory, and never claims SOTA. If a spike pans out you explicitly PROMOTE it into the mainline; if it flops it leaves no trace on the main line. Use when you want to quickly try a hunch (a different head, a quick CNN baseline, a wild idea) without disturbing a running /pursue or skewing the main trajectory."
argument-hint: "<the hunch to spike, e.g. 'try a plain BiLSTM head'>"
---
