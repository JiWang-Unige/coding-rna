---
name: goal-prompt
description: "A5· Generate a Claude Code /goal command **+ companion protocol file** for one experiment iteration. Bakes in Track A/B, screen→scale strategy, 3 execution modes (run-and-evaluate / submit-and-handoff / run-wait-review-pivot), subagent fan-out, data download boundaries, comparability/leakage checks, smart-sbatch, result-log, independent full-scope CLI tri-review, pivot, and evaluator-verifiable completion. Enforces prompt length budget so paste survives Claude Code terminal renderer."
argument-hint: "<iteration description, e.g. 'EXP-A-001: Track A screen Path 1 CRF head, sample_fraction=0.05'>"
---
