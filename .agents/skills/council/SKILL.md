---
name: council
description: "A3.5+· Adversarial multi-agent COUNCIL for Stage A."
---
...
**复用** `/tri-review` 的 CLI 管线：`cluster_config.yaml` 的 `cli_review.reviewers`（A=claude/B=codex/C=antigravity，Reviewer C 经 `.agents/skills/tri-review/scripts/reviewer_c_antigravity.sh`）+ **stdin 喂 prompt 防 ARG_MAX**。
...
- **Uses**: `cluster_config.yaml cli_review`、`.agents/skills/tri-review/scripts/reviewer_c_antigravity.sh`、`scripts/lit_search.py`、MCP exa/anysearch
